...
public class PreferenceScreen extends AppCompatActivity {
    ...
    private void conectar_bt(){
        ...
        new Thread(){
            ...
            public void run() {
                super.run();
                asynk = new Connect(ip.getText().toString(), port.getText().toString(), sem, frec.getText().toString(), buf);
                asynk.execute();
            }
        }.start();
        try {
            ...
            System.out.println("Error semafora");
        }
    }
